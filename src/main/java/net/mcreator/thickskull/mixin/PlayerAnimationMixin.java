	...
	public void setupAnim(PlayerRenderState renderState, CallbackInfo ci) {
		...
		if (!master.equals("thickskullneoforge")) {
			if (!ThickskullneoforgeModPlayerAnimationAPI.animations.isEmpty())
				ThickskullneoforgeModPlayerAnimationAPI.animations.clear();
			return;
		}
		Player player = (Player) renderState.getRenderData(ThickskullneoforgeModPlayerAnimationAPI.ClientAttachments.PLAYER);
		if (player == null)
		...
		if (data.getBooleanOr("ResetPlayerAnimation", false)) {
			...
			ThickskullneoforgeModPlayerAnimationAPI.active_animations.put(player, null);
		}
		...
		ThickskullneoforgeModPlayerAnimationAPI.PlayerAnimation animation = ThickskullneoforgeModPlayerAnimationAPI.active_animations.get(player);
		if (animation == null) {
			animation = ThickskullneoforgeModPlayerAnimationAPI.animations.get(playingAnimation);
			if (animation == null) {
				ThickskullneoforgeMod.LOGGER.info("Attepted to play null animation " + playingAnimation + ", did animations fail to load?");
				return;
			}
			ThickskullneoforgeModPlayerAnimationAPI.active_animations.put(player, animation);
		}
		...
		for (Map.Entry<String, ThickskullneoforgeModPlayerAnimationAPI.PlayerBone> entry : animation.bones.entrySet()) {
			String boneName = entry.getKey();
			ThickskullneoforgeModPlayerAnimationAPI.PlayerBone bone = entry.getValue();
